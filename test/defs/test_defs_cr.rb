# encoding: utf-8
require File.expand_path(File.dirname(__FILE__)) + '/../test_helper'

# This file is generated by the Ruby Holiday gem.
#
# Definitions loaded: data/cr.yaml
class CrDefinitionTests < Test::Unit::TestCase  # :nodoc:

  def test_cr
{Date.civil(2015,1,1) => 'Año Nuevo',
 Date.civil(2015,4,11) => 'Día de Juan Santamaría',
 Date.civil(2009,5,1) => 'Día del Trabajador',
 Date.civil(2009,7,25) => 'Día de la Anexión de Guanacaste',
 Date.civil(2009,8,2) => 'Día de la Virgen de los Angeles',
 Date.civil(2009,8,15) => 'Día de la Madre',
 Date.civil(2009,9,15) => 'Día de la Independencia',
 Date.civil(2009,10,12) => 'Día de las Culturas',
 Date.civil(2016,12,25) => 'Navidad'}.each do |date, name|
  assert_equal name, (Holidays.on(date, :cr)[0] || {})[:name]
end
  end
end
