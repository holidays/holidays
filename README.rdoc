...
After you're satisfied with the YAML file, run rake defs:build_all and rake defs:manifest, which will generate the ruby files that make up the actual code as well as the tests.

Tests can be run using rake tests, use rake -T to see all available tasks.
=== Credits and code
