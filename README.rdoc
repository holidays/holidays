...
For more information, see the notes at the top of the Holidays module.  A super minimal web interface is here:
http://empty-mist-5349.herokuapp.com/
...
=== How to contribute
This library is built into ruby code by parsing the yaml files. So to make changes to any of the definitions, only edit the yaml files.
Tests are also added at the end of the yaml files. Please add tests when you make a change to the definitions!!! Also, some regions could definitely use a little more coverage (especially for the holidays that occur according to a pattern).
Then run rake defs:build_all and rake defs:manifest, which will generate the ruby files as well as the tests.
